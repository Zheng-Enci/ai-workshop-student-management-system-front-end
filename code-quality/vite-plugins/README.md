...
- **功能**: 检查未使用的 CSS 代码（完整功能版）
- **作用**:
  - 使用 PurgeCSS 分析所有 CSS 文件
  - 识别未使用的 CSS 选择器
  - 统计 CSS 使用情况
  - **支持缓存机制**：只在文件变化时重新分析，提高性能
  - **支持并发分析**：可配置并发数，加快分析速度
  - **详细报告**：包含所有文件的完整统计信息
  - 可配置是否在发现未使用 CSS 时阻止开发服务器启动
- **生成报告**: `code-quality/code-quality-reports/css-analysis-report.md`
- **缓存位置**: `.css-analysis/cache/`（自动管理）
...
1. **智能缓存**: 使用文件哈希检测变化，只在文件变化时重新分析
2. **并发分析**: 支持配置并发数（默认5），提高分析速度
3. **完整分析**: 分析所有 CSS 文件，不限制数量
4. **详细报告**: 包含每个文件的详细统计、未使用选择器列表等
