...
<template>
	<el-dialog
		...
		width="90%"
		class="attendance-trend-chart-form-dialog"
		destroy-on-close
		:close-on-click-modal="false"
		:close-on-press-escape="false"
		:append-to-body="true"
		:teleported="true"
		modal-class="attendance-trend-chart-form-overlay"
		@close="closeTrendChartDialog"
	>
		<div class="attendance-trend-chart-form-dialog-content">
			<div v-if="attendanceRecordsLoading" class="attendance-trend-chart-form-loading-container">
				<el-icon class="attendance-trend-chart-form-loading-icon">
					<loading/>
				</el-icon>
				<p>加载中...</p>
			</div>
			<div v-else-if="!StudentManagerPageAttendance_Records_Dialog.state.studentAttendanceRecords || StudentManagerPageAttendance_Records_Dialog.state.studentAttendanceRecords.length === 0" class="attendance-trend-chart-form-no-records">
				<el-icon class="attendance-trend-chart-form-no-records-icon">
					<calendar/>
				</el-icon>
				<p>暂无考勤记录</p>
			</div>
			<div v-else class="attendance-trend-chart-form-container">
				<div class="attendance-trend-chart-form-item">
					<div class="attendance-trend-chart-form-title">签到趋势图</div>
					<div ref="trendDialogChart" class="attendance-trend-chart-form-content"/>
				</div>
			</div>
		</div>
		<template #footer>
			<div class="attendance-trend-chart-form-footer">
				<el-button @click="closeTrendChartDialog" size="large">关闭</el-button>
			</div>
		</template>
	</el-dialog>
</template>
...
/* 移动端适配 */
.attendance-trend-chart-form-dialog {
	max-width: 90%;
}

.attendance-trend-chart-form-dialog :deep(.el-dialog__body) {
	padding: 15px;
	overflow: visible;
}

.attendance-trend-chart-form-dialog :deep(.el-dialog__header) {
	padding: 15px 15px 10px;
}

.attendance-trend-chart-form-dialog :deep(.el-dialog__footer) {
	padding: 10px 15px 15px;
}

.attendance-trend-chart-form-dialog-content {
	width: 100%;
}

/* ==================== 趋势图容器样式 ==================== */
.attendance-trend-chart-form-container {
	width: 100%;
	padding: 10px 5px;
}

.attendance-trend-chart-form-item {
	display: flex;
	flex-direction: column;
	align-items: center;
}

.attendance-trend-chart-form-title {
	font-size: 16px;
	font-weight: bold;
	margin-bottom: 10px;
	text-align: center;
...
.attendance-trend-chart-form-content {
	width: 100%;
	height: 300px;
}

/* ==================== 加载状态样式 ==================== */
.attendance-trend-chart-form-loading-container {
	display: flex;
	flex-direction: column;
	align-items: center;
...
.attendance-trend-chart-form-loading-icon {
	font-size: 32px;
	margin-bottom: 12px;
	animation: attendance-trend-chart-form-rotate 2s linear infinite;
}

@keyframes attendance-trend-chart-form-rotate {
	from {
...
/* ==================== 无数据状态样式 ==================== */
.attendance-trend-chart-form-no-records {
	display: flex;
	flex-direction: column;
	align-items: center;
	justify-content: center;
...
.attendance-trend-chart-form-no-records-icon {
	font-size: 40px;
	margin-bottom: 12px;
	color: #999;
}
...
/* ==================== 底部按钮样式 ==================== */
.attendance-trend-chart-form-footer {
	display: flex;
	justify-content: center;
}

.attendance-trend-chart-form-footer .el-button {
	flex: 1;
}

/* ==================== 夜间模式适配 ==================== */

html.dark .attendance-trend-chart-form-no-records {
	color: #6b7280;
}

html.dark .attendance-trend-chart-form-no-records-icon {
	color: #6b7280;
}

/* ==================== 小屏幕适配 ==================== */
@media screen and (max-width: 375px) {
	.attendance-trend-chart-form-dialog :deep(.el-dialog__title) {
		font-size: 16px;
	}
