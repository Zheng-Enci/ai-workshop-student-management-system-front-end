...
import { setStudentLevel, getStudentLevel } from '@/api/student'
...
export function useStudentLevel() {
   ...
   * @returns {Array} 学生列表
   */
  const getLevelStudents = (level) => {
    return adminStore.students.filter(student => student.identity === level)
  }

  /**
   * 获取学生数量统计
   * @returns {Object} 各等级学生数量
   */
   ...
   */
  const init = async () => {
    const result = await loadStudentLevels()
    return result
  }
