...
- **首页** - 项目介绍和功能导航
- **登录/注册** - 用户认证入口（必须使用邀请码）
- **导航中心** - 功能模块导航入口
- **考勤管理** - 个人考勤记录查看和统计
- **数据看板** - 签到排行榜和成员数据统计
- **积分看板** - 积分排行榜（总积分/签到积分/活动积分）
- **个人中心** - 个人资料管理和密码修改
- **全部成员** - 成员信息展示和搜索
- **学生管理** - 学生信息查询和管理
- **改分记录** - 积分变动记录查询
- **IP监控** - AI坊内IP活跃程度监控
- **管理员后台** - 成员管理、积分调整、补卡处理

### 核心功能

#### 1. 主题切换

- 点击右上角主题切换按钮（ThemeToggle组件）
- 支持日间模式和夜间模式自动切换
- 主题状态持久化存储
...
>
> 本项目历经技术迭代，由最初的 JavaScript 逐步演进为 TypeScript。开发团队始终遵循<strong>"宁愿多写一行新代码，也不修改一行旧代码"</strong>的设计哲学：对运行稳定的既有代码保持敬畏，通过编写新代码实现新需求，由此形成 JavaScript 与 TypeScript 并存的现状。
>
> 这种设计哲学体现了对现有功能的尊重——如果旧代码能正常工作，就不去触碰它，而是通过编写新代码来实现新功能。这确保了系统的稳定性，同时也为项目提供了渐进式迁移的路径。
>
> **希望后续开发者也能秉承这一原则**：优先编写新代码而非修改旧代码，让项目在不断演进中保持稳定。
